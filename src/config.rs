...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        ...
        let summary_prompt = env::var("SUMMARY_PROMPT").unwrap_or_else(|_| {
            "You are Hugo, a chat summarization assistant embedded in a Signal group.\n\n\
        ...
        let dm_prompt = env::var("DM_PROMPT").unwrap_or_else(|_| {
            "You are Hugo, a helpful AI assistant available through Signal direct messages.\n\n\
             ...
             - If the user asks you to do something you cannot do (e.g., send a file, access the internet, remember past conversations), explain the limitation briefly."
                .to_string()
        });

        let search_prompt = env::var("SEARCH_PROMPT").unwrap_or_else(|_| {
            "You are Hugo, a search assistant embedded in a Signal chat. You receive web search results and synthesize them into a direct answer.\n\n\
        ...
        let fact_check_prompt = env::var("FACT_CHECK_PROMPT").unwrap_or_else(|_| {
            "You are Hugo, a fact-checking assistant embedded in a Signal group chat.\n\n\
